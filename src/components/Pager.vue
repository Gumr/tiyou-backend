    ...
    <div class="ms-pager">
        ...
        <div class="row pager-list-wrap" v-if="totalRecordCount > 0">
            <div class="col-md-4 col-sm-12">
                <div class="pager-count mt20">
                    共有<span class="j-ui-pager-totalnum">{{totalRecordCount}}</span>条数据，每页显示 {{pageSize}} 条
                    <select class="input" v-model="pageSize" @change="gotoPage(1, true)">
            ...
            <div class="col-md-8 col-sm-12">
                <ul class="pagination pull-right" v-if='pageCount > 0'>
                    <li class="prev paginate_button" :class='{disabled: isFirst}' title="第一页">
                        <a @click='gotoPage(1)' href='javascript:void(0);'>
                            <i class="fa fa-angle-double-left"></i>
                        </a>
                    </li>
                    <li class="prev paginate_button" :class='{disabled: isFirst}' title="上一页">
                        <a @click='gotoPage(pageNo-1)' href='javascript:void(0);'>
                            <i class="fa fa-angle-left"></i>
                        </a>
                    </li>
                    <li class="paginate_button" v-for='page in pages' :class='{active: page.isActive}' v-bind="{title : '第' + page.pageNo + '页'}">
                        <a  @click='gotoPage(page.pageNo)' href='javascript:void(0);'>
                            {{page.pageNo}}
                        </a>
                    </li>
                    <li class="next paginate_button" :class='{disabled: isLast}' title="下一页">
                        <a @click='gotoPage(pageNo+1)' href='javascript:void(0);'>
                            <i class="fa fa-angle-right"></i>
                        </a>
                    </li>
                    <li class="next paginate_button" :class='{disabled: isLast}' title="最后一页">
    ...
    export default {
        ...
        data: function() {
            return base.assignObj({
                ...
                autoLoad: true,
                url: null,
                getParams: $.noop,  // 由父组件传入的方法覆盖
                onPageLoad: $.noop, // 由父组件传入的方法覆盖
                pageNo: null,
                pages: [],
                pageCount: 100,
        ...
        computed: {
            showCount: function() {
                ...
                return  count && count > 5 ? count : 5;   //显示页码数最小是5
            },
            beginGard: function() {
                return 2;
            },
