    ...
    <div class="col-md-12">
      <div class="portlet light bordered">
        <div class="portlet-body">
            ...
            <form action="#" class="form-horizontal">
              <div class="row flex_group">
                <div class="form-group flex_content">
                  <label class="control-label lable_title_name">主题状态:</label>
                  <div class="">
                ...
                <div class="form-group flex_content">
                  <label class="control-label lable_title_name">主题名称:</label>
                  <div class="">
                    <input type="text" class="form-control" autocomplete="off" placeholder="请输入主题名称搜索" v-model="searchParam.themeName" @keyup.enter="onSearch($event)" />
                  </div>
                </div>
              </div>
              <!-- 清空/搜索 -->
          ...
          <a v-show="checkPermissions(['addTheme'])" @click="onAddEditForm($event)" class="btn green-meadow" href="javascript:;">
            <i class="fa fa-plus-square"></i> 新增主题
          </a>
            ...
            <table class="table table-striped table-bordered table-hover table-checkable order-column dataTable no-footer">
              ...
              <tbody>
                <tr v-for="(obj,idx) in dataList">
                  <td class="text-center">{{obj.themeId || '--'}}</td>
                  ...
                  <td class="text-center">
                    <a v-show="checkPermissions(['updateTheme'])" @click="onAddEditForm($event,obj)" class="btn btn-xs purple" href="javascript:;">
                      <i class="fa fa-edit"></i> 编辑
                    </a>
                    <a v-show="checkPermissions(['updateThemeStatus'])" v-if="obj.status == '0'" @click="updateThemeStatus($event,obj.themeId,1)" class="btn btn-xs green" href="javascript:;">
                      <i class="fa fa-level-up"></i> 上架
                    </a>
                    <a v-show="checkPermissions(['updateThemeStatus'])" v-if="obj.status == '1'" @click="updateThemeStatus($event,obj.themeId,0)" class="btn btn-xs red" href="javascript:;">
...
export default {
  ...
  methods: {
    ...
    //课程上下架操作
    updateThemeStatus: function (event, themeId, status) {
      event.preventDefault();
      let that = this;
      let data = { themeId, status };
      var message;
      if (status == 0) {
        message = '确认下架？'
      } else {
      ...
      bootbox.confirm({
        ...
        callback: function (result) {
          if (result) {
            $.ajax({
              url: "/api/course/updateThemeStatus",
              type: "POST",
